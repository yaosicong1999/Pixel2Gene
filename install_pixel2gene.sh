...
# - pip: most extras to avoid slow/conflicting solves
# ============================================================

ENV_NAME="pixel2gene"
CORE_YML="environment.core.yml"

# Optional: if you want to force CUDA detection in non-GPU nodes
CUDA_OVERRIDE="${CUDA_OVERRIDE:-12.4}"
...
if command -v mamba >/dev/null 2>&1; then
  ...
  SOLVER="conda"
  warn "mamba not found; using conda (may be slower). Consider: conda install -n base -c conda-forge mamba"
fi
...
if conda env list | awk '{print $1}' | grep -qx "${ENV_NAME}"; then
  ...
  ${SOLVER} env create -f "${CORE_YML}" -y
fi
...
# Use current_repodata.json to speed up index fetch (optional but often helps)
export CONDA_REPODATA_FNS="${CONDA_REPODATA_FNS:-current_repodata.json}"

# ============================================================
# Group A: single-cell / ST stack (conda-forge)
# ============================================================
...
set +e
${SOLVER} install -y -n "${ENV_NAME}" -c conda-forge --strict-channel-priority pyarrow=17.0.0
rc=$?
set -e
if [[ $rc -ne 0 ]]; then
  warn "conda/mamba pyarrow solve failed; falling back to pip pyarrow==17.0.0"
  conda run -n "${ENV_NAME}" python -m pip install --no-cache-dir pyarrow==17.0.0
fi
