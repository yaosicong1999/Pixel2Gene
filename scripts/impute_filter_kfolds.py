...
def get_train_test_data(pref_train, pref_test, emb_type="uni"):
    ...
    embs_train = get_embeddings(f'{pref_train}', emb_type=emb_type, smooth=True)
    locs_train = get_locs(f'{pref_train}', target_shape=embs_train.shape[:2])

    if pref_test == pref_train:
        embs_test = embs_train.copy()
    else:
        embs_test = get_embeddings(f'{pref_test}', type='uni', smooth=True)
...
def get_args():
    ...
    parser.add_argument('pref_test', type=str, nargs='?', default=None)
    ...
    parser.add_argument('--emb_type', type=str, default='uni', choices=['hipt', 'virchow2', 'uni', 'uni_pca', 'uni_pca_gene_pca'])
...
def main():
    ...
    if args.pref_test is None:
        pref_test = pref_train
    else:
    ...
    if pref_ref is not None:
        ...
        for i in range(args.ref_n_jobs):
            file_path = f'{args.output}states/{i:02d}/ref_model_weights.pth'
        ...
        if not ref_weights_exist:
            ...
            impute(embs_train=embs_ref, cnts_train=cnts_ref, locs_train=locs_ref,
                    ...
                    n_states=args.ref_n_states, output=args.output,
                    load_saved=False,
                    device=args.device, n_jobs=args.ref_n_jobs,
                    asreference=True)
            print(f"reference training has completed... parameters have been saved in {args.output}states/ref_model_weights.pth")
    ...
    cnts_train, embs_train, locs_train, embs_test = get_train_test_data(pref_train=pref_train, pref_test=pref_test, emb_type=emb_type)
    ...
    if train_mask is not None:
        print("train_mask is provided, filtering steps will be applied...")
        train_mask = load_mask(train_mask)
        assert train_mask.shape[0] == embs_train.shape[0]
        assert train_mask.shape[1] == embs_train.shape[1]
        filter_ind = train_mask[locs_train[:, 0], locs_train[:, 1]] # get the mask indices for locs_train based on mask
        ...
        print("train_mask is not provided, skipping masking steps...") 

    ## loading reference genes if reference is provided
    if args.pref_ref is not None:
        ref_gene_names = read_lines(f'{args.pref_ref}ref-gene-names.txt')
    ...
    impute(
        ...
        n_states=args.n_states, output=args.output, lr=args.lr,
