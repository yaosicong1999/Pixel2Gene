...
def parse_args():
    ...
    parser.add_argument("--obs_label_dir", type=str, default=None, help="Directory containing labels")
    parser.add_argument("--pred_label_dir", type=str, default=None, help="Directory containing labels")
    parser.add_argument("--n_cluster", type=int, default=18, help="Number of clusters")
...
def main():
    ...
    obs_label_dir = args.obs_label_dir
    pred_label_dir = args.pred_label_dir
    n_cluster = args.n_cluster
    top_n = args.top_n

    # Load truth data
    gene_names = read_lines(f"{pred_dir}/predict-gene-names.txt")
    ...
    print(f"Mask shape: {mask.shape}, mask size: {mask.size}")

    # Load ground truth clustering labels 
    if obs_label_dir is not None:
        obs_labels = load_pickle(obs_label_dir + '/labels.pickle')
        print(f"Loading observation labels from {obs_label_dir}/labels.pickle")
        print(f"Observation labels shape: {obs_labels.shape}")
        if obs_labels.shape != mask.shape:
            raise ValueError(f"Labels shape {obs_labels.shape} does not match mask size {mask.size}")
        obs_labels = obs_labels[mask]  # Filter by mask
    else:
        print("No observation labels directory provided.")

    # Load predicted data
    combined_path = f"{pred_dir}combined_full_gene_pickles.npy"
    if os.path.exists(combined_path):
        ...
        for i in tqdm(range(len(gene_names))):
            ...
            x[:, :, i] = load_pickle(f"{pred_dir}cnts-super/{gene_name}.pickle")
    ...
    if pred_label_dir is not None:
        pred_labels = load_pickle(pred_label_dir + '/labels.pickle')
        if pred_labels.shape != mask.shape:
            raise ValueError(f"Predicted labels shape {pred_labels.shape} does not match mask size {mask.size}")
        pred_labels = pred_labels[mask]  # Filter by mask
    else:
        print("No predicted labels directory provided.")
    ...
    if top_n < 50:
        ...
        print(f"Using top {top_n} genes for ICC evaluation. Performing PCA on the data.")
    ...
    # Calculate ICCs
    if top_n is not None:
        truth_icc = calculate_per_pc_iccs(truth, obs_labels)
        print(f"The mean ICC for the truth data is {np.mean(truth_icc):.4f}")
        print(f"The median ICC for the truth data is {np.median(truth_icc):.4f}")
        np.save(f"{obs_label_dir}icc_scores_nclusters_{n_cluster}_top{top_n}.npy", truth_icc)
        pred_icc = calculate_per_pc_iccs(predicted, pred_labels)
        print(f"The mean ICC for the predicted data is {np.mean(pred_icc):.4f}")
        print(f"The median ICC for the predicted data is {np.median(pred_icc):.4f}")
        np.save(f"{pred_label_dir}icc_scores_nclusters_{n_cluster}_top{top_n}.npy", pred_icc)
    else:
        truth_icc = calculate_per_pc_iccs(truth, obs_labels)
        print(f"The mean ICC for the truth data is {np.mean(truth_icc):.4f}")
        print(f"The median ICC for the truth data is {np.median(truth_icc):.4f}")
        np.save(f"{obs_label_dir}icc_scores_nclusters_{n_cluster}.npy", truth_icc)
        pred_icc = calculate_per_pc_iccs(predicted, pred_labels)
        print(f"The mean ICC for the predicted data is {np.mean(pred_icc):.4f}")
        print(f"The median ICC for the predicted data is {np.median(pred_icc):.4f}")
        np.save(f"{pred_label_dir}icc_scores_nclusters_{n_cluster}.npy", pred_icc)        
