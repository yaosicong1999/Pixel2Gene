...
def plot_super(x, truncate=None, save=True, he=None, outfile=None):
    x = x.copy()
    mask = np.isfinite(x)
    ...
    if he is not None:
        assert he.shape[:2] == img.shape[:2], "H&E image and super image should have the same shape"
        img[~mask] = he[~mask]
    if save:
        if outfile is None:
            outfile = 'super_plot.jpg'
