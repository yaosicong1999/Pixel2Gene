...
def main():
    ...
    for condition in ['Downsampled-Observed', 'Downsampled-Imputation']:
        values = x['Pearson Correlation'][condition]
        for v in values:
            data.append({'Metric': 'Pearson', 'Condition': condition, 'Value': v})

    # 2. Spearman
    for condition in ['Downsampled-Observed', 'Downsampled-Imputation']:
        values = x['Spearman Correlation'][condition]
        for v in values:
            data.append({'Metric': 'Spearman', 'Condition': condition, 'Value': v})
    # 3. Cosine Similarity
    for condition in ['Downsampled-Observed', 'Downsampled-Imputation']:
        values = x['Cosine Similarity'][condition]
        for v in values:
            data.append({'Metric': 'Cosine Similarity', 'Condition': condition, 'Value': v})
    ...
    all_groups = [
        'Pearson|Downsampled-Observed',
        'Pearson|Downsampled-Imputation',
        'SPACER',
        'Spearman|Downsampled-Observed',
        'Spearman|Downsampled-Imputation',
        'SPACER',
        'Cosine Similarity|Downsampled-Observed',
        'Cosine Similarity|Downsampled-Imputation',
    ]
    ...
    plt.tight_layout()
    plt.savefig(f'{output}No.{start_idx}-No.{end_idx}_final_violin_correlation.png', dpi=300, bbox_inches='tight')
    plt.close()
