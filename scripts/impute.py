...
def get_train_test_data(pref_train, pref_test):
    ...
    cnts_train = get_gene_counts(f'{pref_train}')
...
def impute(
    ...
    kwargs_list = [
            dict(
                ...
                batch_size=batch_size, epochs=epochs, lr=1e-5,
...
def main():
    ...
    if args.pref_ref is not None:
        ...
        radius_ref = int(read_string(f'{args.ref}radius.txt'))
        ...
        cnts_ref, embs_ref, locs_ref = get_ref_data(pref_ref=args.ref, pref_train=pref_train)
    ...
    if args.pref_ref is not None:
        ref_gene_names = read_lines(f'{args.pref_ref}ref-gene-names')
        ...
        print("reordering the reference genes...", n_ref)
