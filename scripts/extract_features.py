...
def get_embeddings(img, pretrained=True, device='cuda'):
    ...
    if pretrained:
        model256_path = 'vit_checkpoints/vit256_small_dino.pth'
        model4k_path = 'vit_checkpoints/vit4k_xs_dino.pth'
    model = HIPT_4K(
...
def main():
    ...
    save_embeddings(embs, args.prefix + 'embeddings-hipt-raw.pickle')
    ...
    if args.smoothen_method is not None:
        ...
        embs = smoothen_embeddings(
                ...
                device=args.device)
        print('runtime:', int(time()-t0))
        save_embeddings(embs, args.prefix + 'embeddings-hipt-smooth.pickle')
