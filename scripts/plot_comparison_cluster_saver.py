...
def main():
    ...
    if downsampled_dir is not None:
        dirs = [original_dir + 'clusters_truth/', 
                ...
                downsampled_dir + 'clusters_saver/']
        subtitles = ['Original', 'Original-Imputed', 'Downsampled', 'Downsampled-SAVER']
    else:
        dirs = [original_dir + 'clusters_truth/', 
                original_dir + 'clusters_pred_gene_pickle/']
    ...
    if ground_truth_dir is None:        
        base_dir = downsampled_dir + 'saver_cluster_comparison_top12000topc/'
    else:
        base_dir = downsampled_dir + 'saver_cluster_comparison_top12000topc_with_ground_truth/'
    ...
    if downsampled_dir is not None:
        ...
        if os.path.exists(f'{downsampled_pref}saver-imputed-cnts-top{n_top}genes_50pc.npy'):
            print(f'Loading imputed downsampled data from {downsampled_pref}saver-imputed-cnts-top{n_top}genes_50pc.npy')
            imputed_downsampled_flat = np.load(f'{downsampled_pref}saver-imputed-cnts-top{n_top}genes_50pc.npy')
        else:
            cnts_downsampled_imputed, locs_downsampled_imputed = read_parquet_cnts(downsampled_pref+"saver-imputed-", gene_names=gene_names) if os.path.exists(downsampled_pref+'cnts.parquet') else read_tsv_cnts(downsampled_pref, gene_names=gene_names)
            mat_downsampled_imputed = np.full((mask.shape[0], mask.shape[1], cnts_downsampled_imputed.shape[1]), np.nan, dtype=np.float32)
            mat_downsampled_imputed[locs_downsampled_imputed[:,0], locs_downsampled_imputed[:,1], :] = cnts_downsampled_imputed.to_numpy()
            imputed_downsampled_flat = mat_downsampled_imputed.reshape(-1, mat_downsampled_imputed.shape[2])
            del cnts_downsampled_imputed, mat_downsampled_imputed
            imputed_downsampled_flat = reduce_dim(imputed_downsampled_flat, n_components=50)[0]
            np.save(f'{downsampled_pref}saver-imputed-cnts-top{n_top}genes_50pc.npy', imputed_downsampled_flat)
