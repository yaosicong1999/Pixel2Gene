...
def main():
    ...
    for condition in ['Downsampled-Observed', 'Downsampled-Imputation']:
        values = x['SSIM'][condition]
        for v in values:
            data.append({'Metric': 'SSIM', 'Condition': condition, 'Value': v})
    ...
    all_groups = [
        'SSIM|Downsampled-Observed',
        'SSIM|Downsampled-Imputation',
    ...
    plt.tight_layout()
    plt.savefig(f'{output}No.{start_idx}-No.{end_idx}_final_violin_ssim.png', dpi=300, bbox_inches='tight')
    plt.close()
