...
def get_args():
    ...
    parser.add_argument('--ignore_exist', action='store_true')
    parser.add_argument('--clip0', type=float, default=100)
    parser.add_argument('--clip1', type=float, default=100)
...
def main():
    ...
    if mask is not None:
        ...
        he_test = load_image(f'{args.pref_test}he.jpg')
        mask = np.ones([int(he_test.shape[0] / factor), int(he_test.shape[1] / factor)], dtype=bool)

    if not os.path.exists(f'{args.pref_test}locs.tsv'):
        print(f"{args.pref_test}locs.tsv does not exists, skipping SSIM comparison.")
        sys.exit(1)
    locs_test = load_tsv(f'{args.pref_test}locs.tsv')
    ...
    cnts_test = load_tsv(f'{args.pref_test}cnts.tsv')
    cnts_test = cnts_test.iloc[unique_row_indices, :]

    os.makedirs(args.output + 'ssim_full', exist_ok=True)
    os.makedirs(args.output + 'ssim_top100', exist_ok=True)
    ssim_rel_list = []
    ssim_abs_list = []
    gene_name_list = []
    ...
    if os.path.exists(args.output + 'superpixel_ssim_comparison.tsv') and args.ignore_exist:
        df = pd.read_csv(args.output + 'superpixel_ssim_comparison.tsv', sep='\t', index_col=0)
            ...
            if gn in intersected_genes:
                ...
                if ssim_rel_list.__len__() <= 100:
                    plot_img_comparison(img1, img0, gn=gn, width=width, height=height,
                                        save_fig=True, out_dir=[args.output + 'ssim_full/', args.output + 'ssim_top100/'],
                                        title=title, fig_name=gn + '.jpg')
                else:
                    plot_img_comparison(img1, img0, gn=gn, width=width, height=height,
                                        save_fig=True, out_dir=[args.output + 'ssim_full/'],
                                        title=title, fig_name=gn + '.jpg')

        df = pd.DataFrame(
            {'Gene': gene_name_list, 'SSIM-Abs': ssim_abs_list, 'SSIM-Rel': ssim_rel_list})
        df.to_csv(args.output + 'superpixel_ssim_comparison.tsv', sep='\t')
    ...
    plt.savefig(args.output + 'violin_plot_rel_full.png', dpi=200)
    ...
    plt.savefig(args.output + 'violin_plot_abs_full.png', dpi=200)
    ...
    plt.savefig(args.output + 'violin_plot_rel_top100.png', dpi=200)
    ...
    plt.legend()
    plt.title('SSIM by Absolute Value: Predicted vs True\n Top ' + str(100) + ' HVG', fontsize=16)
    plt.savefig(args.output + 'violin_plot_abs_top100.png', dpi=200)
