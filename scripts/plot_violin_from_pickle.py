...
def main():
    ...
    # 3. MS-SSIM
    for condition in ['Downsampled-Observed', 'Downsampled-Imputation']:
        values = x['MS-SSIM'][condition]
        for v in values:
            data.append({'Metric': 'MS-SSIM', 'Condition': condition, 'Value': v})
    ...
    all_groups = [
        ...
        'SPACER',
        'MS-SSIM|Downsampled-Observed',
        'MS-SSIM|Downsampled-Imputation',
        'SPACER',
    ...
    plt.tight_layout()
    plt.savefig(f'{output}No.{start_idx}-No.{end_idx}_final_violin.png', dpi=300, bbox_inches='tight')
    plt.close()
