...
def get_model(
    ...
    ref_weights_file = prefix + 'ref_model_weights.pth'
    ...
    elif os.path.exists(ref_weights_file):
        model = model_class(**model_kwargs)
        print("Now trying to load the reference weights...")
        model.load_reference_weights(torch.load(ref_weights_file))
        print(f'Weights successfully loaded from {ref_weights_file}')
        history = []
    else:
        model = None
        history = []
