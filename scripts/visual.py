...
def plot_labels(
    ...
    if filename is not None:
        ...
        '''
        cmap_arr = cmap(np.arange(n_labels))
        color_map = {i: cmap_arr[i, :3] for i in range(n_labels)}
        fig, ax = plt.subplots(1, 1, figsize=(100, 100), dpi=1)
        for cluster_label, color in color_map.items():
            print("color is ", color)
            ax.scatter(0, 0, color=color, label=f'{cluster_label}', s=0.5)
        ...
        try:
            ...
            pass  # fallback to original order if label not numeric

        # Determine number of columns based on label count
        n_labels = len(labels)
        import math
        nrow = 2
        n_labels = len(labels)
        ncol = math.ceil(n_labels / nrow)

        # Reverse the reordering to compensate for column-major rendering
        # So that legend still appears row-wise
        col_major_indices = list(range(n_labels))
        ...
        for col in range(ncol):
            for row in range(nrow):
                idx = row * ncol + col
                if idx < n_labels:
                    row_major_to_col_major.append(idx)

        # Reorder handles/labels
        handles = [handles[i] for i in row_major_to_col_major]
        labels = [labels[i] for i in row_major_to_col_major]

        # Plot legend
        fig, ax = plt.subplots(figsize=(ncol * 1.5, 2.5), dpi=500)
        fig.legend(handles=handles, labels=labels,
                loc='center', frameon=False,
                prop={'size': 12}, markerscale=5,
                title='Cluster', ncol=ncol)
        ax.remove()
        plt.savefig(filename[:filename.rfind('.')] + '_legend.png', bbox_inches='tight')
        '''
        cmap_arr = cmap(np.arange(n_labels))
        color_map = {i: cmap_arr[i, :3] for i in range(n_labels)}
        fig, ax = plt.subplots(1, 1, figsize=(100, 100), dpi=1)
        ...
        handles = [handles[i] for i in order]
        labels = [f"Cluster {i + 1}" for i in order]
        print("before reordering lables:", labels)

        n_items = len(handles)
        n_rows = 3
        n_cols = math.ceil(n_items / n_rows)

        # Build index list to reorder handles/labels in column-first order
        # so legend displays them row-first
        col_major_indices = []
        ...
        # Reorder
        handles = [handles[i] for i in col_major_indices]
        labels = [labels[i] for i in col_major_indices]
        print("after reordering lables:", labels)


        ncol = math.ceil(len(handles) / 2)
        fig, ax = plt.subplots(figsize=(len(handles) * 0.7, 2), dpi=300)
        fig.legend(
            handles=handles,
            labels=labels,
            loc='upper center',
            ...
            prop={
                ...
                'size': 12},
            markerscale=10,
            handletextpad=0.3,  # 👈 tighter label-dot spacing
            columnspacing=2.0,  # 👈 wider gap between cluster columns
            labelspacing=0.8,  # 👈 vertical spacing between rows
            mode='expand'
        )
        ax.remove()
        plt.savefig(filename[:filename.rfind('.')]+ '_legend.png', bbox_inches='tight', dpi=1000)
...
def plot_spots(
    ...
    cnts = np.clip(cnts, 0, np.percentile(cnts, q=99))
...
def plot_label_masks(labels, prefix, names=None, white_background=True):
    ...
    for idx in range(labs_uniq.__len__()):
        lab = labs_uniq[idx]
        mask = labels == lab
