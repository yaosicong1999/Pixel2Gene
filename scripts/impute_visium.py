...
def get_data(prefix):
    ...
    cnts_test = get_gene_counts(f'{prefix}test-')
    cnts_test = cnts_test[gene_names]
    ...
    locs_test = get_locs(f'{prefix}test-', target_shape=embs_test.shape[:2])

    return cnts_train, cnts_test, embs_train, embs_test, locs_train, locs_test
...
def impute(
        embs_train, cnts_train, locs_train, embs_test, locs_test, radius, epochs, batch_size, prefix,
...
def main():
    args = get_args()
    cnts_train, cnts_test, embs_train, embs_test, locs_train, locs_test = get_data(args.prefix)
    args = get_args()
    ...
    impute(
            ...
            embs_test=embs_test, locs_test=locs_test, radius=radius,
