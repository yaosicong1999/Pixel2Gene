...
from utils import (
        load_tsv, load_parquet, load_pickle, save_pickle, load_image, save_image,
        read_lines)
...
def get_gene_counts(prefix, reorder_genes=True):
    if os.path.isfile(f'{prefix}cnts.parquet'):
        cnts = load_parquet(f'{prefix}cnts.parquet')
    else:
        cnts = load_tsv(f'{prefix}cnts.tsv')
    if reorder_genes:
        cnts = cnts.astype(np.int8)
        order = cnts.var().to_numpy().argsort()[::-1]
        cnts = cnts.iloc[:, order]
    return cnts


def get_embeddings(prefix, emb_type='uni'):
    if emb_type == 'hipt':
        embs = load_pickle(f'{prefix}embeddings-hipt-smooth.pickle')
    if emb_type == 'hipt_raw':
        embs = load_pickle(f'{prefix}embeddings-hipt-raw.pickle')
    if emb_type == 'virchow2':
        embs = load_pickle(f'{prefix}embeddings-virchow2-smooth.pickle')
    if emb_type == 'uni':
        embs = np.load(f'{prefix}embedding_uni.npy')
    if emb_type == 'uni_pca':
        embs = np.load(f'{prefix}embedding_uni-pca.npy')
    if emb_type == 'uni_pca_gene_pca':
        embs = np.load(f'{prefix}embedding_uni-pca-gene-pca.npy')    
