...
def get_train_test_data(pref_train, pref_test):
    ...
    embs_train = get_embeddings(f'{pref_train}', type='hipt', smooth=True)
    locs_train = get_locs(f'{pref_train}', target_shape=embs_train.shape[:2])

    if pref_test == pref_train:
        embs_test = embs_train.copy()
    else:
        embs_test = get_embeddings(f'{pref_test}', type='hipt', smooth=True)
...
def predict(
    ...
    for idx_grp in idx_groups:
        ...
        for i, name in enumerate(name_grp):
            save_pickle(y_grp[..., i], f'{output}cnts-super/{name}.pickle')


def impute(
        embs_train, cnts_train, locs_train, embs_test, radius,
        epochs, batch_size, output,
...
def main():
    ...
    cnts_train, embs_train, locs_train, embs_test = get_train_test_data(pref_train=pref_train, pref_test=pref_test)
    ...
    mask = args.mask
    if mask is not None:
        print("mask is provided, filtering steps will be applied...")
        mask = load_mask(mask)
        assert mask.shape[0] == embs_train.shape[0]
        assert mask.shape[1] == embs_train.shape[1]
        filter_ind = mask[locs_train[:, 0], locs_train[:, 1]] # get the mask indices for locs_train based on mask
        ...
        print("mask is not provided, skipping filtering steps...")
    ...
    impute(
        ...
        n_states=args.n_states, output=args.output,
